...
// with a per-capture `frameSeq` starting at 0, matching CONTRACTS §4.2) plus per-frame VAD results
// for the live meter. The WS client consumes `frame` events; it owns the wire header.
//
...
export interface AudioCaptureOptions {
  ...
  source: MicSource;
  /** Optional VAD tuning; defaults are fine for the meter. */
  vad?: EnergyVADOptions;
}

type CaptureState = 'idle' | 'running' | 'stopped';

/** The capture surface the dictation hook depends on — lets tests inject a fake driver. */
...
export class AudioCapture {
  ...
  /** Subscribe to per-frame VAD results (level + speaking). Returns an unsubscribe fn. */
  onVad(listener: (result: VadResult) => void): () => void {
