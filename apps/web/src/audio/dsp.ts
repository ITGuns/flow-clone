...
//
// Pure DSP primitives — no I/O, no globals, deterministic. Everything here is unit-tested with
// synthetic signals (sine, silence, ramp, step). These are the load-bearing bottom of the capture
// pipeline; the layers above (framing, VAD, capture) build on them.
...
/**
 * Resample a mono Float32 signal to {@link TARGET_SAMPLE_RATE} (16kHz) using linear interpolation
 * (adequate for v1 — CONTRACTS latency budget assumes cheap client DSP).
 *
 * - Identity fast-path when `inputRate === 16000` (returns a defensive copy).
 * - Empty input → empty output.
 * - Throws {@link RangeError} on a non-finite or non-positive `inputRate` (the "wrong input rate"
 *   failure path — a misconfigured mic source must fail loud, not emit garbage audio).
 */
...
/**
 * Convert a mono Float32 signal in [-1, 1] to little-endian PCM16 bytes. Out-of-range samples are
 * hard-clamped (never wrapped). Output length is exactly `input.length * 2`.
 */
...
/**
 * Root-mean-square energy of a PCM16LE frame, normalized to [0, 1] (full-scale = 1.0). Trailing
 * odd byte (if any) is ignored. Empty frame → 0.
 */
