// Energy-based voice activity detector for the HUD level display. Pure and per-frame: give it
// a 640-byte PCM16LE frame, get back {level, speaking}. `level` drives the HUD meter; `speaking`
// is RMS-threshold gated with a hangover tail so brief inter-word gaps don't flicker the state.
//
// This is a display/UX signal, NOT an endpointer — ASR (Deepgram, server-side) owns utterance
// finalization per CONTRACTS §4.3 (`audio.end` on key-up). VAD here never gates what is sent.

import { pcm16Rms } from './dsp';

export interface VadResult {
  /** RMS energy of the frame, normalized to [0, 1]. Feeds the HUD meter directly. */
  level: number;
