// Stateful byte-framer — CONTRACTS.md §4.2. Accepts arbitrary-length PCM16LE byte runs and
// emits exact 640-byte frames, carrying any sub-frame remainder across calls so the frame
// grid never drifts (the WS client needs contiguous frames, no gaps). Emitted frames are
// always fresh copies — mutating the caller's input after `push` never mutates a frame.
...
export class FrameChunker {
   ...
   * Returns `null` when there is no remainder (nothing to flush). Called by `AudioCapture.stop`.
   */
